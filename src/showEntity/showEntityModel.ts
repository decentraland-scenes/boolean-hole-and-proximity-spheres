...
export class ShowEntityModel extends ShowEntitySupport {
  ...
  onPlayDurationStart?: ()=>void
  onPlayDurationEnd?: ()=>void
  ...
  constructor(
    ...
    if (extArgs.idleAnim) {
      ...
      log("anim.constructor start idle anim")
      this.idleAnim.play()
    }
  ...
  playAnimation(
    ...
    if(noLoop){
      if (interval && duration) {
        playOnceAndIdle(this, newAnim, duration,resetAnimation,this.onPlayDurationStart,this.onPlayDurationEnd) 
        ...
        newAnim.looping = false
        log("anim.playAnimation playing and not calling reset ",newAnim)
      ...
      //   newAnim.stop()
      log("anim.playing with reset ",newAnim)
      newAnim.play(resetAnimation)

      handlePlayDuration(this,newAnim,duration,this.onPlayDurationStart,this.onPlayDurationEnd)
...
export function playOnceAndIdle(
  ...
  handlePlayDuration(ent,anim,duration,onStartCallback,onEndCallback)
}

function handlePlayDuration(ent: ShowEntityModel,anim: AnimationState,duration?:number,onStartCallback?:()=>void,onEndCallback?:()=>void){
  if (duration) {
    if(onStartCallback) onStartCallback()
    ent.endAnimTimer.addComponentOrReplace(
      new utils.Delay(duration * 1000, () => {
        log("anim.handlePlayDuration ended",duration)
        // if (ent.lastPlayedAnim) {
        //   ent.lastPlayedAnim.stop()
        // }
        anim.stop()
        ent.playIdleAnimation()
